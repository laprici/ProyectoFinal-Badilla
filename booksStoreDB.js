...
// Añadir un precio aleatorio entre 10 y 200 a cada libro
bd.forEach(book => {
  book.price = +(Math.random() * (200 - 10) + 10).toFixed(2); // Redondea a 2 decimales
  book.stock = Math.floor(Math.random() * (20));
});

// Definir la ruta del archivo donde se guardarán los datos
const filePath = './updated_books.json';

// Guardar el arreglo modificado en un archivo
fs.writeFile(filePath, JSON.stringify(bd, null, 4), (err) => {
  if (err) throw err;
